  ...
  GetStation: function (intent, session, response) {
    ...
    if (_.has(stations, stationName)) {
      ...
      WmataApi.get(endpoint, function (body) {
        ...
        // if there's <= 1 train arriving we don't need any more info from the user; we can just tell them when that
        // train is coming or that no trains are coming. Otherwise we're going to need to respond w/ a question.
