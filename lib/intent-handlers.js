...
const registerIntentHandlers = (intentHandlers) => {
  ...
  intentHandlers.SetHomeStation = (intent, session, response) => {
    console.info('Requested Home Station Name: ' + intent.slots.homeStation.value);
    const stationName = utils.changeStationName(intent.slots.homeStation.value, 'correction');
  ...
  intentHandlers.GetStationHomeSet = (intent, session, response) => {
    storage.loadUserSettings(session, (userSettings) => {
      ...
      _handleGetStation(homeStation, station, response, session);
    });
  };
  ...
  intentHandlers.GetStation = (intent, session, response) => {
    ...
    const station = utils.findStationByName(stationName);
    _handleGetStation(stationName, station, response, session);
  };
...
function _handleGetStation (stationName, station, response, session) {
  if (_.isNil(station)) {
