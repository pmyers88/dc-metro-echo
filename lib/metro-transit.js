...
MetroTransit.prototype.eventHandlers = {
...
MetroTransit.prototype.intentHandlers = {
  ...
  GetStation: function (intent, session, response) {
    ...
    if (_.has(stations, stationName)) {
      ...
      WmataApi.get(props.stationArrivalWmataUrl + stationCode, response, function(body) {
  ...
  GetServiceAdvisories: function(intent, session, response) {
    WmataApi.get(props.serviceAdvisoriesWmataUrl, response, function(body) {
