...
MetroTransit.prototype.intentHandlers = {
  ...
  GetStation: function (intent, session, response) {
    ...
    if (_.has(stations, stationName)) {
      ...
      WmataApi.getWmataResponse(props.stationArrivalWmataUrl + stationCode, response, function(body) {
  ...
  GetServiceAdvisories: function(intent, session, response) {
    WmataApi.getWmataResponse(props.serviceAdvisoriesWmataUrl, response, function(body) {
